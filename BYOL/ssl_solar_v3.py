...
# ==========================================
config = {
    "Dataset Path": "/home/craib/Documents/moco-v3/dataset/original",
    ...
    "Training Epochs": 200,           
    "Warmup Epochs": 10,
    "Batch Size": 32,             # Reduced for ~7.6GB GPU
    ...
    "Accumulation Steps": 16,      # Increased to maintain effective batch size
    
    # Output paths
    "Output Dir": "/home/craib/solar-segmentation-annotation-ssl/Shahzeb/outputs",
}
...
def train():
    ...
    for epoch in range(config['Training Epochs']):
        ...
        if epoch_loss < best_loss and epoch >= config['Warmup Epochs']:
            ...
            save_path = os.path.join(config["Output Dir"], "byol_best.pth")
            torch.save({
                'epoch': epoch + 1,
        ...
        # Checkpoints
        if (epoch + 1) % 20 == 0:
            save_path = os.path.join(config["Output Dir"], f"byol_epoch_{epoch+1}.pth")
            torch.save({
                'epoch': epoch + 1,
                'encoder_state_dict': model.online_encoder.state_dict(),
    ...
    # Final save
    final_encoder_path = os.path.join(config["Output Dir"], "byol_encoder_final.pth")
    log_path = os.path.join(config["Output Dir"], "byol_training_log.csv")
